...
\textbf{Finite chemical potential.} The results of the previous two cases were all for vanishing chemical potential. At finite \(\mu\), lattice computations are plagued by the `sign-problem' which limits the applicability conventional Monte Carlo techniques and makes it difficult to achieve reliable results (see \cite{Ding:2015ona} and references therein). Circumventing this problem using various new algorithms is an active area of research \cite{Sexty:2014dxa}, while at small values of \(\mu\) most of the current results come form Taylor expanding the relevant thermodynamic variable around \(\mu=0\). We also note here that various lattice calculations have led to a tentative prediction of the existence of a critical end point (2nd order transition) in the phase diagram \cite{PhysRevD.67.014505,PhysRevD.68.014507,Fodor:2004nz,deForcrand:2002hgr}, as indicated in Fig.~\ref{fig:QCDphasediag}. In Sec.REF we show how the formalism developed in this thesis is easily extended to finite densities, which becomes relevant in the context of heavy ion collision phenomenology.   
